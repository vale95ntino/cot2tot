...
def plot_as_tree(G: nx.Graph, root: Optional[Any] = None) -> None:
    """
...
def animate_as_tree(
    ...
    initial_node_size: float = max(
        MIN_NODE_SIZE, min(DEFAULT_NODE_SIZE * (10 / (num_nodes + 5)), MAX_NODE_SIZE)
    )
    ...
    for node in nodes_in_order:
        ...
        thresholds_passed: int = sum(
            1 for t in SHRINK_THRESHOLDS if len(visible_nodes) >= t
        )
        ...
        for step in range(1, FADE_STEPS + 1):
            fade_alpha: float = step / FADE_STEPS
